...
void amd_enable_mem_encrypt(const struct cpuinfo_x86 *c)
{
    ...
    if (!c->cpu_core_id) {
	printk(XENLOG_INFO "AMD MemEncrypt is supported, c-bit mask:0x%lx)\n",
	       pte_c_bit_mask);
	if (cpu_has_sev)
	    printk(XENLOG_INFO "SEV ASID usable range [0x%x - 0x%x]\n",
