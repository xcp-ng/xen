...
unsigned long __ro_after_init *hvm_asid_bitmap;
unsigned long __ro_after_init *hvm_reclaim_asid_bitmap;
...
/* TODO: Recheck that VMX is using this properly after the rework */
unsigned int __init hvm_asid_init(int nasids)
{
    ...
    hvm_asid_bitmap = xvzalloc_array(unsigned long,
                                    BITS_TO_LONGS(data->max_asid));
    if ( !hvm_asid_bitmap )
        return -ENOMEM;

    hvm_reclaim_asid_bitmap = xvzalloc_array(unsigned long,
                                            BITS_TO_LONGS(data->max_asid));
    ...
    set_bit(0, hvm_asid_bitmap);

     /* As ASID 0 is reserved, we start the counting from 1 */
    data->next_asid = find_first_zero_bit(hvm_asid_bitmap, data->max_asid ) + 1;

    return 0;
}

/* TODO: hvm_asid_flush_domain_asid is removed, rework this function */
...
bool hvm_asid_domain_create(struct hvm_domain_asid *asid)
{
    ...
    /* Ensure next_asid is within bounds */
    ...
    {
        /* Check if there are any ASIDs to reclaim */
        if ( !bitmap_empty(hvm_reclaim_asid_bitmap, data->max_asid + 1) )
        {
            /* Merge hvm_reclaim_asid_bitmap into hvm_asid_bitmap */
                for ( unsigned int i = 0; i <= data->max_asid; ++i )
                {
                    if ( test_bit(i, hvm_reclaim_asid_bitmap) )
                        set_bit(i, hvm_asid_bitmap);
                }

            /* Clear the reclaim bitmap */
            bitmap_zero(hvm_reclaim_asid_bitmap, data->max_asid + 1);

            /* Find the next available ASID */
            data->next_asid = find_first_zero_bit(hvm_asid_bitmap, data->max_asid) + 1;
        }
    else
    {
...
void hvm_domain_asid_destroy(struct domain *d)
{
