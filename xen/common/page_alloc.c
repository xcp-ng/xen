...
unsigned long domain_adjust_tot_pages(struct domain *d, nodeid_t node, long pages)
{
                                 ...
                                 unsigned long pages)
{
    ...
    if ( node != NUMA_NO_NODE )
    {
        avail_pages = node_avail_pages[node] -
                                        ...
                                        const struct domain *d)
{
    ...
    for ( ; ; )
    {
        ...
        BUG_ON(per_node_outstanding_claims[node] > node_avail_pages[node]);
...
void free_domheap_pages(struct page_info *pg, unsigned int order)
{
    ...
    else
    {
        ...
        if ( likely(d) && likely(d != dom_cow) )
        {
            ...
            drop_dom_ref = !domain_adjust_tot_pages(d, page_to_nid(pg), -(1 << order));
