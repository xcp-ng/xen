...
#define SEV_CMDRESP_RESP              (1UL << 31)
#define SEV_CMDRESP_IOC               (1UL << 0)
...
static unsigned int sev_cmd_buffer_len(int cmd)
{
    switch (cmd) {
        case SEV_CMD_INIT:                     
...
static int _sev_do_cmd_sync(struct amd_sp_dev *sp, int cmd, void *data, unsigned int *psp_ret)
{
    unsigned int cbuff_pa_low, cbuff_pa_high, cmd_val, buf_len, cmdresp, i;
    int rc;
...
int sev_do_cmd(int cmd, void *data, unsigned int *psp_ret, bool poll)
{
...
static int __init sp_get_state(struct amd_sp_dev *sp, int *state, unsigned int *err)
{
