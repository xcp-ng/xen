...
static void cf_check do_sp_irq(void *data);
static DECLARE_SOFTIRQ_TASKLET(sp_irq_tasklet, do_sp_irq, NULL);

/*
TODO_AS:
 - define own irq chip to take them once guest is started?
 - look why PV guest messes with ASP PCI device
 */

/*FIXME_AS: make them command line tunables */
static unsigned int asp_timeout_val = 30000;
static unsigned long long asp_sync_delay = 1000ULL;
static int asp_sync_tries = 4;

void sp_cmd_lock(void)
{
    spin_lock(&_sp_cmd_lock);
}

void sp_cmd_unlock(void)
{
...
static void cf_check invalidate_cache(void *unused)
{
    wbinvd();
}

int __sev_do_cmd(int cmd, void *data, int *psp_ret)
{
    ...
    /* FIXME_AS: If the timer triggers here the device will be set offline */

    wait_event(sp->cmd_in_progress, sp->cmd_timeout || sp->intr_rcvd);

    stop_timer(&sp->cmd_timer); /* unconditionally stop timer (no effect on stoped) */
    ...
    else
    {
        ASSERT(sp->cmd_timeout);
            /* set the device off and disable interrupts */
        sp->state = SEV_STATE_UNINIT;
        writel(0, sp->io_base + sp->vdata->inten_reg);
            /* FIXME_AS: choose the new master and destroy the device*/
        rc = -EIO;
    }
    return rc;
}

static int __sev_do_cmd_sync(struct amd_sp_dev *sp, int cmd, void *data, int *psp_ret)
{
    unsigned int cbuff_pa_low, cbuff_pa_high, cmd_val;
...
int sev_do_cmd(int cmd, void *data, int *psp_ret)
{
    ...
    sp_cmd_lock();
    rc = __sev_do_cmd_sync(sp, cmd, data, psp_ret);
    sp_cmd_unlock();

    return rc;
}

int sev_do_cmd_sync(int cmd, void *data, int *psp_ret)
{
    struct amd_sp_dev *sp = amd_sp_master;
    int rc;
    sp_cmd_lock();
...
static void cf_check do_sp_cmd_timer(void *data)
{
    struct amd_sp_dev *sp = (struct amd_sp_dev*)data;

    asprintk(("ASP: command timeout\n"));
...
static void cf_check sp_interrupt_handler(
    int irq, void *dev_id, struct cpu_user_regs *regs)
{
    struct amd_sp_dev *sp = (struct amd_sp_dev*)dev_id;
    uint32_t status;

    asprintk(("ASP: got interrupt\n"));
...
static int sp_get_state(struct amd_sp_dev *sp, int *state, int *err)
{
    struct sev_user_data_status status;
    int rc;

    rc = __sev_do_cmd_sync(sp, SEV_CMD_PLATFORM_STATUS, &status, err);
    if ( rc )
        return rc;
...
static int __init sp_get_api_version(struct amd_sp_dev *sp)
{
    ...
    asprintk(("asp-%pp: api-%d.%d state:0x%x \n", &sp->pdev->sbdf, sp->api_major,
	      sp->api_minor, sp->state));

    return 0;
}

static int sp_update_firmware(struct amd_sp_dev *sp)
{
        /*
         * FIXME_AS: do nothing on that stage
         */
    return 0;
}
...
{
        /*
         * FIXME_AS: allocate TMP memory area for SEV-ES
         */
    return 0;
}
...
static int __init sp_do_init(struct amd_sp_dev *sp)
{
    ...
    rc = __sev_do_cmd_sync(sp, SEV_CMD_INIT, &data, &err);
    if ( rc )
...
static int __init sp_df_flush(struct amd_sp_dev *sp)
{
    ...
    rc = __sev_do_cmd_sync(sp, SEV_CMD_DF_FLUSH, NULL, &err);
    if ( rc )
...
static int __init sp_dev_init(struct amd_sp_dev *sp)
{
    ...
    printk(XENLOG_INFO "asp-%pp: init done\n", &sp->pdev->sbdf);
    return 0;
}
...
static int __init sp_init_irq(struct amd_sp_dev *sp)
{
    ...
    asprintk(("asp-%pp: enabled interrupts %d\n", &sp->pdev->sbdf, irq));
...
static int  __init sp_dev_create(struct pci_dev *pdev, struct psp_vdata *vdata)
{
    ...
    printk(XENLOG_INFO "discovered asp-%pp \n", &sp->pdev->sbdf);
...
static int __init cf_check amd_sp_probe(void)
{
    ...
    struct  amd_sp_dev *sp;
     ...
     {
	 printk(XENLOG_INFO "AMD SEV isn't supported on the platform\n");
	 return 0;
     }
