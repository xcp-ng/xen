                     ...
                     char *protocol)
{
    ...
    if ( opt_mem_pnode != XC_NUMA_NO_NODE )
        dom->memflags = XENMEMF_exact_node(opt_mem_pnode);
                           ...
                           unsigned long *store_mfn, unsigned long *console_mfn)
{
    ...
    unsigned int memflags =
        opt_mem_pnode != XC_NUMA_NO_NODE ? XENMEMF_exact_node(opt_mem_pnode)
                                         : 0;
    ...
    r = xc_domain_restore(xch, fd, domid,
                          ...
                          XC_STREAM_PLAIN, &cbs, -1, memflags);
    if ( r )
